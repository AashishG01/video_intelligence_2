...
import { UserPlus, Trash2, AlertTriangle, User, CheckSquare, Square, X, Loader2, Settings2 } from 'lucide-react';
import api from '../api';
...
const WatchlistManager = () => {
    ...
    const [isManageCatModalOpen, setIsManageCatModalOpen] = useState(false); // NEW: Dynamic Category Modal
    ...
    return (
        <div className="p-8 bg-slate-50 min-h-screen relative">
            ...
            {/* 1. Category Management Modal (Admin Only) */}
            ...
            {/* 2. SMART Modal (Handles both Enroll and Edit) */}
            <EnrollSubjectModal
            ...
            {/* 3. DOSSIER POP-UP MODAL (Read-Only View) */}
            {activeDossier && (
                <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white rounded-[24px] shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col md:flex-row relative">
                        ...
                        <div className="w-full md:w-3/5 p-8 flex flex-col relative">
                            ...
                            <div className={`inline-flex items-center self-start px-3 py-1 rounded-md text-xs font-black uppercase mb-6 ${activeDossier.risk_level === 'Extreme' ? 'bg-red-100 text-red-700' :
            ...
            <div className="flex space-x-2 mb-8 overflow-x-auto pb-2 custom-scrollbar items-center">

                {/* Admin Manage Lists Button */}
                {user?.role === 'admin' && (
                    <button
                        onClick={() => setIsManageCatModalOpen(true)}
                        className="px-4 py-2.5 rounded-xl text-sm font-bold bg-slate-200 text-slate-700 hover:bg-slate-300 transition-all flex items-center shadow-inner whitespace-nowrap mr-2"
                    >
                ...
                <button
                    ...
                    className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all whitespace-nowrap ${selectedCategory === 'All' ? 'bg-slate-900 text-white' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-50'
                        }`}
                >
                    All ({subjects.length})
                </button>
                {categories.map(cat => {
                    ...
                    return (
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-xl text-sm font-bold border transition-all whitespace-nowrap ${isSelected ? 'shadow-md text-white' : 'bg-white text-slate-500 hover:bg-slate-50'
                                }`}
                    ...
                    {filteredSubjects.map(subject => {
                        ...
                        return (
                            <div
                                ...
                                className={`group cursor-pointer bg-white border rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all ${isSelected ? 'border-blue-500 ring-2 ring-blue-500/50 scale-[0.98]' : 'border-slate-200 hover:border-slate-300'
                                    }`}
                            >
